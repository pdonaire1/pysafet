...
#define PASSTWO   16
    ...
    QString result = tr("¡Se ha enviado un <b>correo</b>  a su cuenta. Siga el enlace que allí aparece para terminar el proceso de registro!");
    int typecheck = 0;
    ...
    if( users.contains(account)) {
        ...
        QString myaction = QString("operacion:Agregar_usuario Nombre_cuenta_usuario: %1 Contraseña_usuario: %2 "
               "Nombre_completo_usuario: %3 Rol_del_Usuario: %4")
                .arg(account)
                .arg(newhash)
                .arg(fullname + " " + email)
                .arg("Invitado");
...
bool MainWindow::toInputUsers(const QString& action) {
    ...
    /**
      No especificar porque es un parametro en la funcion Generar_ticker
      **/
